...
<SeoHead title="Sign Up" description={`Create your ${siteName} account`} url="/auth/register" noindex={true} />
...
<section class="min-h-[80vh] flex items-center justify-center px-4 py-10">
	<div class="w-full max-w-md">
		...
		<div class="text-center mb-8">
			...
			<p class="text-surface-400">Join {siteName} to bookmark articles and more</p>
		</div>
		...
		<div class="bg-surface-900 border border-surface-800 rounded-2xl p-6 space-y-5">
			...
			<!-- Social Login -->
			<div class="space-y-3">
				<button type="button" onclick={handleGoogleLogin} disabled={!!oauthLoading} class="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-surface-800 hover:bg-surface-700 disabled:opacity-60 text-white text-sm font-medium transition-all border border-surface-700">
				...
				<button type="button" onclick={handleGithubLogin} disabled={!!oauthLoading} class="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-surface-800 hover:bg-surface-700 disabled:opacity-60 text-white text-sm font-medium transition-all border border-surface-700">
			...
			<form onsubmit={(e) => { e.preventDefault(); handleRegister(); }} class="space-y-4">
				<div>
					<label for="name" class="block text-sm text-surface-300 mb-1.5">Display Name</label>
					<div class="relative">
						<User size={16} class="absolute left-3 top-1/2 -translate-y-1/2 text-surface-500" />
						<input id="name" type="text" bind:value={displayName} required placeholder="Your name"
				...
				<div>
					...
					<div class="relative">
						...
						<input id="password" type={showPassword ? 'text' : 'password'} bind:value={password} required placeholder="Min 6 characters"
						...
						<button type="button" onclick={() => showPassword = !showPassword} class="absolute right-3 top-1/2 -translate-y-1/2 text-surface-500 hover:text-surface-300" aria-label={showPassword ? 'Hide password' : 'Show password'} aria-pressed={showPassword}>
			...
			<p class="text-sm text-center text-surface-400">
				...
				<a href="/auth/login" class="text-accent-400 hover:text-accent-300">Sign in</a>
			</p>
		</div>
	</div>
</section>
