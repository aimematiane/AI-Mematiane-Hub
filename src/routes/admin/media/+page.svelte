<script>
	...
	import { Image, Upload, Trash2, Search, FolderOpen, Grid, List, Copy, ExternalLink, X, Check } from '@lucide/svelte';
	...
	let uploadForm = $state({ url: '', filename: '', alt_text: '', folder: '/' });
	...
	async function uploadMedia() {
		...
		await fetch('/admin/media', {
			method: 'POST',
			body: new URLSearchParams({ ...uploadForm, action: 'upload' })
		});

		showUploadModal = false;
		uploadForm = { url: '', filename: '', alt_text: '', folder: '/' };
		saving = false;
		goto('/admin/media');
	}
	...
	async function saveEdit() {
		await fetch('/admin/media', {
			method: 'POST',
			body: new URLSearchParams({ id: editingMedia.id, ...editForm, action: 'updateMeta' })
		});
		showEditModal = false;
		editingMedia = null;
		goto('/admin/media');
							...
							<tr class="hover:bg-surface-800/50">
								...
								<td class="px-6 py-4">
									<div class="flex items-center justify-end gap-2">
										<button onclick={() => startEdit(item)} class="p-2 rounded-lg hover:bg-surface-700 text-surface-400">
											Edit
										</button>
...
{#if showUploadModal}
	<div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" onclick={(e) => { if (e.target === e.currentTarget) showUploadModal = false; }}>
		<div class="bg-surface-900 border border-surface-800 rounded-2xl p-6 w-full max-w-md">
			<h3 class="text-lg font-semibold text-white mb-4">Add Media File</h3>
			<p class="text-sm text-surface-400 mb-4">Enter the URL of an external image or file.</p>
			<div class="space-y-4">
				<div>
					<label class="block text-sm text-surface-300 mb-1.5">URL *</label>
					<input type="url" bind:value={uploadForm.url} placeholder="https://..." class="w-full px-4 py-2.5 rounded-xl bg-surface-800 border border-surface-700 text-white text-sm" />
				</div>
				<div>
					<label class="block text-sm text-surface-300 mb-1.5">Filename</label>
					<input type="text" bind:value={uploadForm.filename} placeholder="auto-detected from url" class="w-full px-4 py-2.5 rounded-xl bg-surface-800 border border-surface-700 text-white text-sm" />
				</div>
				<div>
					<label class="block text-sm text-surface-300 mb-1.5">Alt Text</label>
					<input type="text" bind:value={uploadForm.alt_text} placeholder="description for accessibility" class="w-full px-4 py-2.5 rounded-xl bg-surface-800 border border-surface-700 text-white text-sm" />
				</div>
				<div>
					<label class="block text-sm text-surface-300 mb-1.5">Folder</label>
					<input type="text" bind:value={uploadForm.folder} placeholder="/" class="w-full px-4 py-2.5 rounded-xl bg-surface-800 border border-surface-700 text-white text-sm" />
				</div>
			</div>
			<div class="flex gap-3 mt-6">
				...
				<button onclick={uploadMedia} disabled={saving || !uploadForm.url} class="flex-1 px-4 py-2.5 rounded-xl bg-accent-500 hover:bg-accent-600 disabled:opacity-50 text-white font-medium text-sm">{saving ? 'Adding...' : 'Add'}</button>
			</div>
		</div>
	</div>
{/if}
